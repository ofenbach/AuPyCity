...
from windows.compressor_ui import CompressorUI
from windows.denoiser_ui import DenoiserUI
from windows.normalizer_ui import NormalizerUI
from libs import wavefile, presets

# Makes loading images easier
from windows.reverb_ui import ReverbUI

OUTPUT_PATH = Path(__file__).parent
ASSETS_PATH = OUTPUT_PATH / Path("./tkinterdesigner/bottom_bar/")
...
class BottomBarUI:
    ...
    def show_normalizer(self):
        if self.context.current_window != "normalizer":
            try:
                ...
                print("[ERROR] deleting other effect windows")
                print(e)
    ...
    def show_equalizer_window(self):
        ...
        if self.current_window_opened == "none":
            ...
            self.image_vintage = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/vintage.png").resize((64, 92)))
            self.button_vintage = Button(image=self.image_vintage, borderwidth=0, highlightthickness=0,
                                         command=lambda: presets.apply_vintage(self.sliders_eq), relief='flat')
            self.button_vintage.place(x=450, y=497, width=64, height=92)

            self.image_eguitar = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/electric_guitar.png").resize((64, 92)))
            self.button_eguitar = Button(image=self.image_eguitar, borderwidth=0, highlightthickness=0,
                                         command=lambda: presets.apply_eguitar(self.sliders_eq), relief='flat')
            self.button_eguitar.place(x=541, y=497, width=64, height=92)

            self.image_aguitar = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/acoustic_guitar.png").resize((64, 92)))
            self.button_aguitar = Button(image=self.image_aguitar, borderwidth=0, highlightthickness=0,
                                         command=lambda: presets.apply_aguitar(self.sliders_eq), relief='flat')
            self.button_aguitar.place(x=632, y=497, width=64, height=92)

            self.image_piano = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/piano.png").resize((64, 92)))
            self.button_piano = Button(image=self.image_piano, borderwidth=0, highlightthickness=0,
                                        command=lambda: presets.apply_piano(self.sliders_eq), relief='flat')
            self.button_piano.place(x=723, y=497, width=64, height=92)

            self.image_vocals = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/vocals.png").resize((64, 92)))
            self.button_vocals = Button(image=self.image_vocals, borderwidth=0, highlightthickness=0, bg="#212121",
                                        command=lambda: presets.apply_vocals(self.sliders_eq), relief='flat')
            self.button_vocals.place(x=814, y=497, width=64, height=92)

            self.image_bassy = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/bass.png").resize((64, 92)))
            self.button_bassy = Button(image=self.image_bassy, borderwidth=0, highlightthickness=0, bg="#212121",
                                       command=lambda: presets.apply_bassy(self.sliders_eq), relief='flat')
            self.button_bassy.place(x=901, y=497, width=64, height=92)

            self.image_bands = ImageTk.PhotoImage(Image.open("tkinterdesigner/presets/frequencies.png").resize((585, 14)))
